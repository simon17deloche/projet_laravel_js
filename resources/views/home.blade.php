                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @foreach($articles as $article)
                        <a href="{{ route('article.show',$article) }}"
                           class="bg-gray-100 mb-8 rounded-lg group block rounded-lg p-6 bg-white ring-1 ring-slate-900/5 shadow-lg space-y-3 hover:bg-gray-200 hover:ring-gray-200">
                            <h1 class="text-2xl"> Titre de l'Article : {{ $article->title }}</h1>
                            <hr>
                            <p class="">Auteur de l'article : {{ $article->user->name }}</p>
